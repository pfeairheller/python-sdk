"""Essr Session Manager for MCP servers."""
...
from kep.essr.server import Request
from kep.essr.server import Response
from kep.essr.server.types import Receive, Scope, Send

from mcp.server.lowlevel.server import Server as MCPServer
from mcp.server.essr import (
...
class ESSRSessionManager:
    """
    Manages Essr sessions with optional resumability via event store.

    This class abstracts away the complexity of session management, event storage,
    and request handling for Essr transports. It handles:
    ...
    Important: Only one ESSRSessionManager instance should be created
    ...
    def __init__(
        ...
        self._server_instances: dict[str, EssrServerTransport] = {}
    ...
    async def run(self) -> AsyncIterator[None]:
        ...
        ESSRSessionManager instance cannot be reused after this
        ...
        async with self._run_lock:
            if self._has_started:
                raise RuntimeError(
                    "ESSRSessionManager .run() can only be called "
        ...
        async with anyio.create_task_group() as tg:
            ...
            self._task_group = tg
            logger.info("Essr session manager started")
            try:
                yield  # Let the application run
            finally:
                logger.info("Essr session manager shutting down")
